angular.module('libraryOrganizer')
    .controller('settingsController', function($scope, $mdDialog, $mdToast, $http, vm) {
        ...
        $scope.pushSettings = function() {
            ...
            var settings = [];
            for (var group in $scope.settings) {
                for (var s in $scope.settings[group].settings) {
                    setting = {
        ...
        $scope.updateSettings = function() {
            ...
            $http({
                ...
                for (var group in settinggroups) {
                    var settings = [];
                    for (var setting in settinggroups[group]) {
                        settings.push({
        ...
        $scope.saveLibraries = function() {
            ...
            for (var i in $scope.ownedLibraries) {
                ...
                for (var j in $scope.ownedLibraries[i].editusers) {
                ...
                for (j in $scope.ownedLibraries[i].checkoutusers) {
                    added = false;
                    for (var k in $scope.ownedLibraries[i].user) {
                ...
                for (j in $scope.ownedLibraries[i].viewusers) {
                    added = false;
                    for (var l in $scope.ownedLibraries[i].user) {
                        if ($scope.ownedLibraries[i].user[l].id == $scope.ownedLibraries[i].viewusers[j].id) {
                            $scope.ownedLibraries[i].user[l].permission += 1;
                            added = true;
                        }
                    }
                    if (!added) {

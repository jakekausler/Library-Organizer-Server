...
def check_book(cases, books, currentCase, currentShelf, x, shelfWidth, shelfHeight, i):
	...
	checkingIdx = i + 1
	print()
	print("Starting stack at x: {}".format(x))
	checkingIdx, _ = form_stack(books, stack, stackSidewaysInfo, stack[-1], x, currentY, shelfHeight, checkingIdx)
	stackWidth = 0
	for b in stack:
		stackWidth += b.Width
	print()
...
def form_stack(books, stack, stackSidewaysInfo, previousSideways, currentX, currentY, shelfHeight, checkingIdx):
	innerX = 0
	...
	while checkingIdx < len(books):
		...
		elif currentY + books[checkingIdx].Width > shelfHeight:
		...
		# if it can be placed sideways, add it to the stack and form the stack on top of it. When the stack on top of it is done, move past it
			...
			checkingIdx += 1
			checkingIdx, ix = form_stack(books, stack, stackSidewaysInfo, stack[-1], currentX + innerX, currentY+stack[-1].Width, shelfHeight, checkingIdx)
			innerX += ix - currentX
	return checkingIdx
...
def fill_cases_with_lookahead(cases, books):
	...
	while i < len(books):
		...
		if not books[i].Checked:
			shouldBreakShelf, shouldBreakDivider = check_book(cases, books, currentCase, currentShelf, x, shelfWidth, shelfHeight, i)
			if shouldBreakShelf:
				x = 0
				currentShelf += 1
				...
				shelfHeight = cases[currentCase].Shelves[currentShelf].Height - STACKED_CASE_PADDING
				while oversized:
					books.insert(i, oversized.pop(0))
				continue
...
def drawCases(screen, cases):
	font=pygame.font.SysFont('timesnewroman',  int(20*MULTIPLIER))
	for cn, case in enumerate(cases):
		for sn, shelf in enumerate(case.Shelves):
			for idx, book in enumerate(shelf.Books):
				# print(entry.Book)
				rect = pygameRect(book, cases, cn, sn)
				pygame.draw.rect(screen, BLACK, rect, 1)
				screen.blit(font.render(str(idx), True, BLACK), rect)

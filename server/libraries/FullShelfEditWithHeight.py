...
SESSION = "MTY3NjQ4NTE3N3xEdi1CQkFFQ180SUFBUkFCRUFBQVlfLUNBQUVHYzNSeWFXNW5EQmtBRjJ4cFluSmhjbmx2Y21kaGJtbDZaWEp6WlhOemFXOXVCbk4wY21sdVp3dzBBREphVTAwNFluTTRibmQ1UjJwcWFsTkZibEpuUjNneWRXOWxjSEZqZVcxcU5raFVhbmhoZGpoclowOTZXV2RoWTJKSFZRPT18Y96EKSufWmZKqR0hjwViO4-IXWvQEkPdxjGeDZkZQI8="
...
def sort_books(books):
	...
	# 	b.Contributors.sort(key=sort_contributors)
	# 	if i < len(books) - 1 and i >= nextCheck and check_next_contributor_equality(books, i):
	# 		checkIndex = i+1
	# 		while checkIndex < len(books) - 1 and check_next_contributor_equality(books, i):
	# 			checkIndex += 1
	# 		nextCheck = checkIndex + 1
	# 		books[i:checkIndex] = sorted(books[i:checkIndex], key=lambda o: o.Height)
...
def fill_shelf(case, shelf, books, index, oversized):
	...
	currentDivider = 0
	placingOversized = False
	previousOversized = None
	currentXOnOversized = 0
	while index < len(books) and useWidth+x <= shelf.Width-shelf.PaddingRight:
		if not placingOversized and oversized:
			placingOversized = check_oversized_placement(case, shelf, books, index, useWidth, x, dividers, currentDivider, oversized)
			if placingOversized:
				previousOversized = place_oversized(shelf, books, x, oversized)
				currentXOnOversized = 0
		index, x, currentDivider, placingOversized, previousOversized, currentXOnOversized = fill_book(case, shelf, books, index, currentBook, useWidth, x, oversized, dividers, currentDivider, placingOversized, previousOversized, currentXOnOversized)
		# print("Book ({}, {}, {}) end x: {} (useWidth {})".format(case.CaseNumber, shelf.ShelfNumber, index, x, useWidth))
		if index < len(books):
			currentBook = books[index]
			useWidth = get_use_width(case, currentBook)
	# print("End x for shelf ({},{}): {}. useWidth + x: {}. End of Shelf: {}".format(case.CaseNumber, shelf.ShelfNumber, x, useWidth+x, shelf.Width-shelf.PaddingRight))
	return index

def fill_book(case, shelf, books, index, currentBook, useWidth, x, oversized, dividers, currentDivider, placingOversized, previousOversized, currentXOnOversized):
	if placingOversized and previousOversized and currentXOnOversized + useWidth <= previousOversized.Height:
		currentBook.PreviousSideways = previousOversized
		add_book(shelf, books, index, currentBook, useWidth, x + currentXOnOversized, previousOversized.Width)
		return index + 1, x, currentDivider, True, previousOversized, currentXOnOversized + useWidth
	elif placingOversized and previousOversized:
		x += previousOversized.Height
	if currentBook.Height > shelf.Height:
		oversized.append(currentBook)
		return index + 1, x, currentDivider, False, None, 0
	x, currentDivider = check_fill_dividers(case, x, useWidth, dividers, currentDivider)
	y = perform_sideways(shelf, books, index, currentBook, x, dividers, currentDivider)
	useWidth = add_book(shelf, books, index, currentBook, useWidth, x, y)
	return index + 1, x + useWidth, currentDivider, False, None, 0

def check_oversized_placement(case, shelf, books, index, useWidth, x, dividers, currentDivider, oversized):
	oversizedBook = oversized[0]
	y = oversizedBook.Width
	maxStackHeight = shelf.Height - STACKED_CASE_PADDING
	if x + oversizedBook.Height < shelf.Width-shelf.PaddingRight:
		currentWidthOnBook = books[index].Width
		checkIndex = index
		booksToPlaceOnTop = []
		while currentWidthOnBook < oversizedBook.Height and checkIndex < len(books):
			if books[checkIndex].Height + y >= maxStackHeight:
				return False
			booksToPlaceOnTop.append(books[checkIndex])
			checkIndex += 1
			currentWidthOnBook += books[checkIndex].Width
		print(oversizedBook.Title, oversizedBook.Height, x + oversizedBook.Height, shelf.Width-shelf.PaddingRight)
		for book in booksToPlaceOnTop:
			print("\t", book.Title, book.Width)
		return True
	return False

def place_oversized(shelf, books, x, oversized):
	book = oversized[0]
	del oversized[0]
	book.IsSideways = True
	add_book(shelf, books, -1, book, 0, x, 0)
	return book

def add_book(shelf, books, index, currentBook, useWidth, x, y):
	currentBook.x = x
	currentBook.y = y
	if index == -1: # We don't need to figure out the useWidth
		shelf.Books.append(currentBook)
	elif currentBook.IsSideways:
...
def perform_sideways(shelf, books, index, currentBook, x, dividers, currentDivider, y=0):
	if (not currentBook.IsSideways and
		...
		maxStackHeight = shelf.Height - STACKED_CASE_PADDING
		stackHeight = y + currentBook.Width
		checkIndex = index + 1
...
def draw_title(b, x, y, caseCanvas, previousSidewaysHeight):
	...
	if not b.IsSideways:
		text_x = x+b.Width/2
		text_y = y - previousSidewaysHeight - 4
		rotation = -90
...
def draw_book_box(c, b, x, y, caseCanvas):
	...
	if b.IsSideways:
		...
		if b.PreviousSideways:
			previousSideways = b.PreviousSideways
			while previousSideways != None:
				previousSidewaysHeight += previousSideways.Width
				previousSideways = previousSideways.PreviousSideways
			caseCanvas.add(caseCanvas.rect((x, y-b.Height-previousSidewaysHeight), (b.Width, b.Height), id='book-{}'.format(b.ID), class_='bookcase-book', stroke='black', fill='{}'.format(b.SpineColor)))
		else:
